...
```
User <-> React Frontend <-> FastAPI Backend <-> Gemini 2.5 Flash
                              |
                     PostgreSQL + Qdrant
                              |
                      Memory Pipeline
...
```
Single compound SQL query (1 round-trip):
...
Parallel:
  ...
  [Thread 2] Compound SQL: profile + foresight + messages + keyword search (1 round-trip)
Then:
...
Regex-only (no LLM call) — parses date references from user queries for filtered search
---
...
agentic_layer/
  ...
  temporal_parser.py         # Regex-based date extraction (no LLM)
  vectorize_service.py       # Gemini embedding (single + batch)
  profile_commands.py        # Remember/forget direct profile editing

backend/
  app.py                     # FastAPI app, CORS, lifespan (warmup + periodic ingestion)
  schemas.py                 # Pydantic request/response models
  gemini.py                  # Single Gemini client + time calculator tool
  ...
  routes/
    ...
    chat.py                  # /chat streaming endpoint (SSE with timings)
    query.py                 # /query endpoint (hybrid search with token count)
    stats.py                 # /ping health check + system stats
...
### 4. Docker (local PostgreSQL + Qdrant)
