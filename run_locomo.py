...
def ingest_conversation(sample: dict):
    """Ingest all sessions sequentially using v3 hybrid pipeline."""
    from main_v3 import ingest_v3
    ...
    print(f"\n=== INGESTING (v3 hybrid, per-session sequential) ===")
    ...
    for i, session_key in enumerate(sessions):
        ...
        try:
            ingest_v3(msgs, source_id=source_id, current_date=session_date,
...
def main():
    ...
    if args.reset:
        print("\nResetting databases...")
        from utils.recreate import reset_postgres, reset_qdrant
        import db
        import vector_store
        db.init_schema()
        reset_postgres()
    ...
    if args.fast and not args.normal:
        ...
        # Default: run both fast then normal
